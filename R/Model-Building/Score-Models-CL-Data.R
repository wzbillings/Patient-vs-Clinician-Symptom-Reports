...
fmla_list <-
	list(
		...
		lasso_uw = paste0(lasso_score_vars, collapse = ":"),
...
glm_list |>
	readr::write_rds(
		here::here("Results", "Data", "Cl", "Score-Models",
							 "Score-GLMs.Rds")
	)
...
der_scores <-
	...
	dplyr::mutate(
		...
		lasso_var = as.numeric(dplyr::if_all(tidyselect::all_of(lasso_score_vars)))
	) |>
	dplyr::rename_with(
		.cols = lasso_var,
		.fn = \(x) paste0(lasso_score_vars, collapse = ":")
	) |>
...
# One of the models is rank-deficient, but this doesn't matter for a
# predictive model so I suppressed the warning.
suppressWarnings(
risk_models <-
	der_scores |>
	dplyr::mutate(
...
readr::write_rds(
	risk_to_join,
	file = here::here("Results", "Data", "Cl", "Score-Models",
